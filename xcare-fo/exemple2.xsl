...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:variable name="serviceInfos" select="document('https://raw.githubusercontent.com/champix56/asdaex_base_form/master/xcare-fo/dataServices/cardio.xml')"/>
	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="A4_default_first_header_all_footer">
				...
				<fo:flow flow-name="xsl-region-body">
					<fo:block>
					<xsl:copy-of select="$serviceInfos"/>
					</fo:block>
	...
	<xsl:template name="cartouche-service">
		<fo:static-content flow-name="xsl-region-start">
			<fo:block>
				...
				<fo:block xsl:use-attribute-sets="richetext-bold">
	...
	<xsl:template match="section/personnels/personnel">
		<fo:block><xsl:value-of select="nom/@civ"/> <xsl:value-of select="nom"/></fo:block>
	</xsl:template>
</xsl:stylesheet>
